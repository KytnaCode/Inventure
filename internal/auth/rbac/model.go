...
type RoleModel struct {
	...
	Name string `gorm:"size:255;uniqueIndex:idx_role_name,where:deleted_at IS NULL,priority:1"`
	...
	ResourceType string `gorm:"size:24;uniqueIndex:idx_role_name,where:deleted_at IS NULL,priority:3"`
	...
	ResourceID uuid.UUID `gorm:"size:36;uniqueIndex:idx_role_name,where:deleted_at IS NULL,priority:2"`
}
...
type AccessModel struct {
	...
	RoleID uuid.UUID `gorm:"size:36;uniqueIndex:idx_resource,where:deleted_at IS NULL,priority:2"`
	...
	ResourceType string `gorm:"size:24;uniqueIndex:idx_resource,where:deleted_at IS NULL,priority:3"`

	// ResourceID is the ID of the resource the access is granting permissions on.
	ResourceID uuid.UUID `gorm:"size:36;uniqueIndex:idx_resource,where:deleted_at IS NULL,priority:1"`
}
